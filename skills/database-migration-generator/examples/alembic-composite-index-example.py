...
def upgrade():
    ...
    op.create_index(
        "idx_users_email_status", "users", ["email", "status"], postgresql_concurrently=True
    )


def downgrade():
    # Drop index concurrently (safe rollback)
    op.drop_index("idx_users_email_status", table_name="users", postgresql_concurrently=True)
